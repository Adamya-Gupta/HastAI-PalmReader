...
export default function Readings() {
  ...
  const getIconSize = () => {
    if (!mounted || windowWidth === 0) return 35;
    return windowWidth < 768 ? 30 : 35;
  };

  const getRadius = () => {
    if (!mounted || windowWidth === 0) return 220;
    return windowWidth < 768 ? 150 : 220;
  };

  const getInnerIconSize = () => {
    if (!mounted || windowWidth === 0) return 30;
    return windowWidth < 768 ? 20 : 30;
  };

  const getInnerRadius = () => {
    if (!mounted || windowWidth === 0) return 180;
    return windowWidth < 768 ? 120 : 180;
  };

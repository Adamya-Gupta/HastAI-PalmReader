...
export default function LandingPage() {
  ...
  const handModelRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    setMounted(true);
    
    // Delay Three.js rendering to ensure proper hydration
    ...
    return () => {
      clearTimeout(timer);
      // Additional cleanup when navigating away
      setShouldRenderModel(false);
    };
  }, []);

  // Handle route changes and cleanup
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        setTimeout(() => setShouldRenderModel(true), 100);
      }
    };
  ...
  return (
    ...
    >
      ...
      <section className="flex flex-col lg:flex-row justify-between px-8 md:py-30 max-w-7xl mx-auto md:h-screen">
        ...
        <div 
          ref={handModelRef}
          className="h-[450px] hidden md:flex items-center xl:mr-20"
        >
          {mounted && shouldRenderModel && <HandModel />}
        </div>
      </section>
